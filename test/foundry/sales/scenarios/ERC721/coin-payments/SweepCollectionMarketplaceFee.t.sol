...
contract BundledSaleMarketplaceFee is PaymentProcessorSaleScenarioBase {
...
collectionLevelOffer: false,
        ...
        assertEq(approvedPaymentCoin.balanceOf(sellerEOA), 0.95 ether * numItemsInBundle);
        assertEq(approvedPaymentCoin.balanceOf(buyerEOA), 0 ether);
        assertEq(approvedPaymentCoin.balanceOf(address(marketplaceMock)), 0.05 ether * numItemsInBundle);
        assertEq(approvedPaymentCoin.balanceOf(address(royaltyReceiverMock)), 0 ether * numItemsInBundle);
    }
}